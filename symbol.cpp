...
int
Symbol::getIndex(const char *symbolName)
{
...
int
Symbol::newIndex(const char *symbolName)
{
...
void
Symbol::setValue(int n, int value)
{
...
void
Symbol::dumpList(void)
{
	printf("numSymbols = %d\n", numSymbols);
	for (int i = 0; i < numSymbols; ++i) {
		printf("Symbol %d  %-6s %3d\n", i, symbolTable[i].name, symbolTable[i].intValue);
	}
}

void
Symbol::unitTest(void)
{
	int a = newIndex("A");
	int b = newIndex("B");
	int c = newIndex("C");
